...
def syllable_count(word_instance,k):
   ...
   for i in range(k) :
       if (len(word_instance[i]) < 5) :
            syllable[i] = 1
       if (len(word_instance[i]) >= 5 and len(word_instance[i]) < 8) :
            syllable[i] = 2
       if (len(word_instance[i]) >=8 and len(word_instance[i]) < 13) :
            syllable[i] = 3  
...
def publishResults(sentence_count, count, net_count, word_length, xx2):
   ...
   f.write("<li>FKRA (Flesch-Kincaid) Reading Level - (alt): "+str(round(fkra,1))+"</li>")
